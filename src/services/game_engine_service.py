...
class GameEngineService:
    def __init__(self, seed: int | None = None) -> None:
        ...
        self._evaluator = RoundEvaluator()
    ...
    def start_round(self, session: GameSessionModel) -> None:
        active_candidates = [
            ...
            and player.occupied_cell is not None
        ]
        if not active_candidates:
            ...
            self._logger.warning(
                "Round start rejected because there are no active participants in valid cells"
            )
            return
        ...
        if len(active_candidates) == 1:
            ...
            self._logger.info(
                "Round start immediately finished with winner %s", session.winner_id
            )
            return
        ...
        for player in session.players.values():
            player.active_in_round = (
                player.tracking_state is PlayerTrackingState.ACTIVE
                and player.occupied_cell is not None
            )
        ...
        self._logger.info(
            "Round %s started with participants=%s",
    ...
    def randomize_flashing_pattern(self, session: GameSessionModel) -> None:
        for cell_index in session.grid.cell_states:
            session.grid.cell_states[cell_index] = self._random.choice(
                (CellState.GREEN, CellState.RED)
            )

    def lock_round(self, session: GameSessionModel) -> None:
        session.round_state.phase = RoundPhase.LOCKED
        session.round_state.timer_remaining = (
            session.round_state.timings.reaction_window_seconds
        )
        for cell_index in session.grid.cell_states:
            session.grid.cell_states[cell_index] = self._random.choice(
                (CellState.GREEN, CellState.RED)
    ...
    def begin_check(self, session: GameSessionModel) -> None:
        ...
        session.round_state.timer_remaining = (
            session.round_state.timings.lock_delay_seconds
        )
        session.status_message = (
            f"Round {session.round_state.round_number} checking positions"
        )
        self._logger.info(
            "Round %s checking positions", session.round_state.round_number
        )

    def begin_preparing_next_round(self, session: GameSessionModel) -> None:
        session.round_state.phase = RoundPhase.PREPARING
        session.round_state.timer_remaining = (
            session.round_state.timings.inter_round_delay_seconds
        )
        session.status_message = (
            f"Next round in {session.round_state.timer_remaining:0.1f}s"
        )
        self._logger.info(
            "Preparing next round after round %s", session.round_state.round_number
        )
    ...
    def tick(self, session: GameSessionModel, delta_seconds: float) -> None:
        ...
        round_state.timer_remaining = max(
            0.0, round_state.timer_remaining - delta_seconds
        )
        ...
        if round_state.phase is RoundPhase.FLASHING:
            ...
            while (
                ...
                and round_state.timer_remaining > 0.0
            ):
                round_state.flash_elapsed_seconds -= interval
                self.randomize_flashing_pattern(session)
            ...
            if round_state.timer_remaining <= 0.0:
                ...
                session.status_message = (
                    f"Round {round_state.round_number} checking positions"
                )
            ...
            if round_state.timer_remaining <= 0.0:
                ...
                session.status_message = (
                    f"Next round in {round_state.timer_remaining:0.1f}s"
                )

    def current_mapped_players(
        ...
        return [
            MappedPlayerState(
                ...
                ambiguous=player.occupied_cell is None
                and player.standing_point is not None,
    ...
    def evaluate_round(
        ...
        players_list = (
            ...
            else self.current_mapped_players(session)
        )
        round_state = session.round_state
        round_state.phase = RoundPhase.RESULTS
        round_state.timer_remaining = round_state.timings.elimination_display_seconds

        evaluation = self._evaluator.evaluate(session, players_list)
        round_state.survivor_ids = list(evaluation.survivors)
        round_state.eliminated_ids = list(evaluation.eliminated)

        for participant_id in round_state.participant_ids:
            session_player = session.players[participant_id]
            eliminated = participant_id in evaluation.eliminated
            ...
            session_player.tracking_state = (
                ...
                if eliminated
                else PlayerTrackingState.ACTIVE
            )
    ...
    def revive_player(self, session: GameSessionModel, player_id: str) -> None:
        ...
        player.tracking_state = (
            PlayerTrackingState.ACTIVE
            if player.standing_point is not None
            else PlayerTrackingState.MISSING
        )
    ...
    def reset_session(self, session: GameSessionModel) -> None:
        ...
        session.app_state = (
            AppState.CALIBRATED
            if session.calibration.is_valid
            else AppState.CAMERA_READY
        )
    ...
    def _refresh_round_outcome(self, session: GameSessionModel) -> None:
        ...
        round_state.survivor_ids = [
            player_id
            for player_id in round_state.participant_ids
            if player_id not in round_state.eliminated_ids
        ]
    ...
    def build_render_state(self, session: GameSessionModel) -> RenderState:
        ...
        for player in session.players.values():
            if (
                player.tracking_state is PlayerTrackingState.MISSING
                and player.standing_point is None
                and not player.active_in_round
            ):
                continue
