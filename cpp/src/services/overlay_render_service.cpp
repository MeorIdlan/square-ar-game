    ...
    const CalibrationModel* calibration)
{
    ...
    draw_grid_overlay(renderer, state, calibration);
    draw_player_markers(renderer, state, calibration);
    ...
    const CalibrationModel* calibration)
{
    ...
    for (const auto& [idx, cell_state] : state.grid_cells) {
        ...
        auto corners = project_floor_cell(inv_h, fx, fy, cell_w, cell_h);
    ...
    d3d::Color4 color, float radius)
{
    ...
    renderer.draw_circle(dst[0].x, dst[0].y, radius, color);
}
    ...
    float width, float height)
{
    ...
    for (const auto& player : state.players) {
        auto label = std::wstring(player.player_id.begin(), player.player_id.end())
                   + L": "
                   + std::wstring(player.status_text.begin(), player.status_text.end());
    ...
    float x, float y, float w, float h) const
{
    ...
    return {{
        {img_pts[0].x, img_pts[0].y},
        {img_pts[1].x, img_pts[1].y},
        {img_pts[2].x, img_pts[2].y},
        {img_pts[3].x, img_pts[3].y}
    }};
}
