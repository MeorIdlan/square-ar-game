...
namespace sag::ui
{
    ...
    {
        camera_device_names_ = std::move(device_names);
        // Clamp selection index to valid range
        selected_device_index_ = std::clamp(config_ ? config_->camera.camera_index : 0,
                                            0, std::max(0, static_cast<int>(camera_device_names_.size()) - 1));
    }

    void OperatorWindow::set_camera_formats(std::vector<CameraProfile> formats)
    {
        camera_formats_ = std::move(formats);
        selected_format_index_ = 0;
        // Try to pre-select the format matching current config
        if (config_)
        {
            CameraProfile current{config_->camera.frame_width, config_->camera.frame_height,
                                  config_->camera.target_fps};
            for (int i = 0; i < static_cast<int>(camera_formats_.size()); ++i)
            {
                if (camera_formats_[i] == current)
                {
                    selected_format_index_ = i;
                    break;
                }
            }
        }
    ...
    void OperatorWindow::draw_controls(const GameSessionModel &session)
    {
        if (ImGui::CollapsingHeader("Controls", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            if (ImGui::SliderInt("Rows", &rows, 2, 12))
            {
                ...
                if (callbacks_.set_grid_rows)
                    callbacks_.set_grid_rows(rows);
            }
            ...
            if (ImGui::SliderInt("Columns", &cols, 2, 12))
            {
                ...
                if (callbacks_.set_grid_columns)
                    callbacks_.set_grid_columns(cols);
            }

            ImGui::Separator();

            // ── Camera device ─────────────────────────────────────────────────
            if (!camera_device_names_.empty())
            {
                // Keep selected_device_index_ in sync with config
                selected_device_index_ = std::clamp(config_->camera.camera_index, 0,
                                                    static_cast<int>(camera_device_names_.size()) - 1);
                std::vector<const char *> name_ptrs;
                name_ptrs.reserve(camera_device_names_.size());
                for (const auto &n : camera_device_names_)
                    name_ptrs.push_back(n.c_str());

                if (ImGui::Combo("Camera##device", &selected_device_index_,
                                 name_ptrs.data(), static_cast<int>(name_ptrs.size())))
                {
                    config_->camera.camera_index = selected_device_index_;
                    if (callbacks_.set_camera_index)
                        callbacks_.set_camera_index(selected_device_index_);
                }
            }
            else
            {
                ...
                if (ImGui::InputInt("Camera Index", &cam_idx))
                {
                    ...
                    if (callbacks_.set_camera_index)
                        callbacks_.set_camera_index(cam_idx);
                }
            }

            // ── Camera format ─────────────────────────────────────────────────
            if (!camera_formats_.empty())
            {
                selected_format_index_ = std::clamp(selected_format_index_, 0,
                                                    static_cast<int>(camera_formats_.size()) - 1);
                std::vector<std::string> fmt_labels;
                fmt_labels.reserve(camera_formats_.size());
                for (const auto &f : camera_formats_)
                    fmt_labels.push_back(f.label());
                std::vector<const char *> fmt_ptrs;
                fmt_ptrs.reserve(fmt_labels.size());
                for (const auto &l : fmt_labels)
                    fmt_ptrs.push_back(l.c_str());

                if (ImGui::Combo("Format##camfmt", &selected_format_index_,
                                 fmt_ptrs.data(), static_cast<int>(fmt_ptrs.size())))
                {
                    const CameraProfile &chosen = camera_formats_[selected_format_index_];
                    config_->camera.frame_width = chosen.width;
                    config_->camera.frame_height = chosen.height;
                    config_->camera.target_fps = chosen.fps;
                    if (callbacks_.set_camera_format)
                        callbacks_.set_camera_format(chosen);
                }
            }
            else
            {
                // No formats enumerated yet — show current settings as read-only hint
                ImGui::Text("Format: %dx%d @ %d FPS",
            ...
            {
                if (callbacks_.reconnect_camera)
                    callbacks_.reconnect_camera();
            }
            ...
            if (ImGui::InputInt("Projector Monitor", &monitor))
            {
                ...
                if (callbacks_.set_projector_monitor)
                    callbacks_.set_projector_monitor(monitor);
            }
            ...
            {
                if (config_->aruco_dictionary == aruco_dicts[i])
                {
                    aruco_idx = i;
                    break;
                }
            ...
            if (ImGui::Combo("ArUco Dictionary", &aruco_idx, aruco_dicts, IM_ARRAYSIZE(aruco_dicts)))
            {
                ...
                if (callbacks_.set_aruco_dictionary)
                    callbacks_.set_aruco_dictionary(aruco_dicts[aruco_idx]);
            }
            ...
            {
                if (callbacks_.calibrate)
                    callbacks_.calibrate();
            }
            ImGui::SameLine();
            if (ImGui::Button("Reset Calibration"))
            {
                if (callbacks_.reset_calibration)
                    callbacks_.reset_calibration();
            }

            if (ImGui::Button("Start Round"))
            {
                if (callbacks_.start_round)
                    callbacks_.start_round();
            }
            ImGui::SameLine();
            if (ImGui::Button("Lock Round"))
            {
                if (callbacks_.lock_round)
                    callbacks_.lock_round();
            }
            ImGui::SameLine();
            if (ImGui::Button("Force Next Round"))
            {
                if (callbacks_.force_next_round)
                    callbacks_.force_next_round();
            }

            if (ImGui::Button("Reset Session"))
            {
                if (callbacks_.reset_session)
                    callbacks_.reset_session();
            }
            ImGui::SameLine();
            if (ImGui::Button("Save Settings"))
            {
                if (callbacks_.save_settings)
                    callbacks_.save_settings();
            }
    ...
    void OperatorWindow::draw_timing_controls()
    {
        if (ImGui::CollapsingHeader("Timing", ImGuiTreeNodeFlags_DefaultOpen))
        {
            auto timing_slider = [this](const char *label, const char *field, float &value)
            {
                ...
                {
                    if (callbacks_.set_timing)
                        callbacks_.set_timing(field, value);
                }
            };
    ...
    void OperatorWindow::draw_overrides(const GameSessionModel &session)
    {
        if (ImGui::CollapsingHeader("Player Overrides"))
        {
            ...
            if (!player_ids.empty())
            {
                ...
                {
                    if (callbacks_.revive_player)
                        callbacks_.revive_player(pid);
                }
                ImGui::SameLine();
                if (ImGui::Button("Eliminate"))
                {
                    if (callbacks_.eliminate_player)
                        callbacks_.eliminate_player(pid);
                }
                ImGui::SameLine();
                if (ImGui::Button("Remove"))
                {
                    if (callbacks_.remove_player)
                        callbacks_.remove_player(pid);
                }
            }
            else
            {
