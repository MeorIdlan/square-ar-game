...
namespace sag::ui
{
    ...
    bool ProjectorWindow::create_on_monitor(int monitor_index, HINSTANCE hInstance)
    {
        ...
        Logger::info(std::format("ProjectorWindow: {} monitor(s) detected, requesting index {}",
                                 monitors.size(), monitor_index));
        if (monitors.empty())
        {
            Logger::error("No monitors enumerated");
            return false;
        }

        int idx = std::clamp(monitor_index, 0, static_cast<int>(monitors.size()) - 1);
        if (idx != monitor_index)
            Logger::warn(std::format("Monitor index {} clamped to {}", monitor_index, idx));
        const auto &rect = monitors[idx].rect;
        int w = rect.right - rect.left;
        int h = rect.bottom - rect.top;
        Logger::info(std::format("Projector on monitor {} at ({},{}) {}x{}",
                                 idx, rect.left, rect.top, w, h));
