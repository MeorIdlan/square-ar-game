...
namespace sag
{
    ...
    public:
        CameraWorker(ICameraService &camera, LatestValue<FramePacket> &output, int target_fps)
            : camera_(camera), output_(output), interval_ms_(target_fps > 0 ? 1000 / target_fps : 33)
        {
        }

        void set_target_fps(int fps)
        {
            interval_ms_ = std::max(1, fps > 0 ? 1000 / fps : 33);
            fps_tracker_.reset();
        }
    ...
    private:
        void capture_loop()
        {
            ...
            packet.actual_fps = fps_tracker_.record_frame();
