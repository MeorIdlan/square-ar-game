...
void Application::shutdown()
{
    ...
    // Stop workers
    if (camera_worker_) camera_worker_->stop();
    if (vision_worker_) vision_worker_->stop();

    // Release hardware
    if (camera_service_) camera_service_->release();
    if (pose_service_)   pose_service_->close();
...
void Application::process_pose_result()
{
    ...
    for (const auto& m : mapped) {
        if (m.standing_point) ++mapped_count;
        if (m.in_bounds) ++in_bounds_count;
    }
...
ui::OperatorCallbacks Application::make_operator_callbacks()
{
    ...
    cb.calibrate          = [this]() { calibrate(); };
    cb.reset_calibration  = [this]() { reset_calibration(); };
    ...
    cb.set_timing = [this](const std::string& field, float value) {
        ...
        if (field == "flashing_duration_seconds")       { t.flashing_duration_seconds = value; rt.flashing_duration_seconds = value; }
        else if (field == "flash_interval_seconds")     { t.flash_interval_seconds = value; rt.flash_interval_seconds = value; }
        else if (field == "reaction_window_seconds")    { t.reaction_window_seconds = value; rt.reaction_window_seconds = value; }
        else if (field == "lock_delay_seconds")         { t.lock_delay_seconds = value; rt.lock_delay_seconds = value; }
        else if (field == "elimination_display_seconds") { t.elimination_display_seconds = value; rt.elimination_display_seconds = value; }
        else if (field == "inter_round_delay_seconds")  { t.inter_round_delay_seconds = value; rt.inter_round_delay_seconds = value; }
        else if (field == "missed_detection_grace_seconds") { t.missed_detection_grace_seconds = value; rt.missed_detection_grace_seconds = value; }
    };
